...
if (!function_exists('get_domain_paths')) {
     ...
     * 拼接图片域名
     * @param $paths
     * @param bool $multiple  是否是多图
     *
     * @return array|string  多图时返回数组
     */
    function get_domain_paths($paths, $multiple = false)
    {
        $domain = config('upload.cdnurl') ? config('upload.cdnurl') : request()->domain();
        $preg = "/^http(s)?:\\/\\/.+/";
        if($multiple){
            if($paths){
                return  array_map(function ($path) use ($domain, $preg) {
        ...
        if(preg_match($preg, $paths)) {
            return $paths;
        }else {
            return $paths ? $domain . trim($paths) : "";
        }
    }
}
...
if(!function_exists('get_qr_code')){
     ...
     * @param string $type 二维码类型（文件夹名）
     * @param string $name 文件名（不带后缀）
     *
     * @return string
     */
    function get_qr_code(string $url, string $type, string $name)
    {
        $fileName  = $name . ".png";
        $uploadDir = '/uploads/'. $type .'/';
        $dirPath = Env::get('root_path') . "/public" . $uploadDir;
        ...
        return request()->domain() . $uploadDir . $fileName;
    }
}