...
class Upload
{
    ...
    {
        $this->config = Config::pull('upload');
        $this->chunkDir = Env::get('runtime_path') . 'chunks';
        if ($file) {
            $this->setFile($file);
        }
    }
    ...
    public function chunk($chunkid, $chunkindex, $chunkcount, $chunkfilesize = null, $chunkfilename = null, $direct = false)
    {
        ...
        $destDir = Env::get('runtime_path') . 'chunks';
    ...
    public function upload($savekey = null)
    {
        ...
        $destDir = Env::get('root_path') . 'public' . str_replace('/', DIRECTORY_SEPARATOR, $uploadDir);
