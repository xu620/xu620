...
class Config extends Backend
{
    ...
    protected function refreshFile()
    {
        ...
        file_put_contents(
            Env::get('config_path') . DIRECTORY_SEPARATOR . 'site.php',
    ...
    public function selectpage()
    {
        ...
        $_REQUEST = ['showField' => $setting['field'], 'keyField' => $setting['primarykey'], 'custom' => $custom, 'searchField' => [$setting['field'], $setting['primarykey']]];
    ...
    {
        $tableList = [];
        $dbname = \think\facade\Config::get('database.database');
     ...
     * 查询达达余额
     */
    public function dadaBalance()
    {
        $dada = new \dada\DaDaService();
        $balance = $dada->balance();
        if($balance){
            $this->success('请求成功', '', $balance);
        }
        $this->error('请求失败');
    }
