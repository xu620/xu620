...
class Install extends Command
{
    ...
    {
        define('INSTALL_PATH', Env::get('app_path') . 'admin' . DIRECTORY_SEPARATOR . 'command' . DIRECTORY_SEPARATOR . 'Install' . DIRECTORY_SEPARATOR);
        // 覆盖安装
    ...
    protected function installation($mysqlHostname, $mysqlHostport, $mysqlDatabase, $mysqlUsername, $mysqlPassword, $mysqlPrefix, $adminUsername, $adminPassword, $adminEmail = null, $siteName = null)
    {
        ...
        // 后台入口文件
        $adminFile = Env::get('root_path') . 'public' . DIRECTORY_SEPARATOR . 'admin.php';
        ...
        Db::name('admin')->where('username', 'admin')->update($data);

        // 变更前台默认用户的密码,随机生成
        $newSalt = substr(md5(uniqid(true)), 0, 6);
        $newPassword = md5(md5(Random::alnum(8)) . $newSalt);
        Db::name('user')->where('username', 'admin')->update(['password' => $newPassword, 'salt' => $newSalt]);

        // 修改后台入口
        $adminName = '';
        if (is_file($adminFile)) {
            $adminName = Random::alpha(10) . '.php';
            rename($adminFile, Env::get('root_path') . 'public' . DIRECTORY_SEPARATOR . $adminName);
        }

        //修改站点名称
        if ($siteName != config('site.name')) {
            Db::name('config')->where('name', 'name')->update(['value' => $siteName]);
            $configFile = Env::get('config_path') . DIRECTORY_SEPARATOR . 'site.php';
            $config = include $configFile;
            $configList = Db::name("config")->select();
