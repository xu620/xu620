define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            $('input[name="row[dada_source_id]"]').parent().next().append('<a class="btn btn-info dada_balance" style="margin-right: 10px">' + __('Query balance') + '</a><a class="btn btn-success dada_recharge">' + __('Dada recharge') + '</a>');
            $(document).on("click", ".dada_balance", function () {
                Fast.api.ajax("general/config/dadaBalance",
                    function (ret) {
                        layer.alert("运费账户余额：" + ret.deliverBalance + "<br/>红包账户余额：" + ret.redPacketBalance);
                        return false;
                    },function(){
                        layer.msg('查询失败');
                        return false;
            ...
            $(document).on("click", ".dada_recharge", function () {
                var that = this;
                Layer.prompt({title: __('Please input recharge amount'), formType: 0}, function (value, index) {
                    Layer.close(index);
                    Fast.api.ajax({
                        url: "general/config/dadaRecharge",
                        data: $(that).closest("form").serialize() + "&amount=" + value
                    },function(ret){
                        window.open(ret);
                        return false;
                    },function(){
